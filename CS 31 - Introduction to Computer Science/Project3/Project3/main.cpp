...
int main()
{
    ...
    int badb;
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("D9/D1/", instrs, badb) == 2  &&  instrs == "xxx"  &&  badb == 1);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("Cb2/", instrs, badb) == 2  &&  instrs == "xxx"  &&  badb == 1);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("D", instrs, badb) == 1  &&  instrs == "xxx"  &&  badb == -999);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("D6/D", instrs, badb) == 1  &&  instrs == "xxx"  &&  badb == -999);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    ...
    assert(translateTune("////", instrs, badb) == 0  &&  instrs == "    "  &&  badb == -999);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("/D///D/", instrs, badb) == 0  &&  instrs == " H  H"  &&  badb == -999);
    
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("D/E/F/G/A/", instrs, badb) == 0  &&  instrs == "HJKLQ"  &&  badb == -999);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("D2/D3/D/D4/D5/", instrs, badb) == 0  &&  instrs == "X.HHR"  &&  badb == -999);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("D#/Db/", instrs, badb) == 0  &&  instrs == "@!"  &&  badb == -999);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("D#4/Db4/", instrs, badb) == 0  &&  instrs == "@!"  &&  badb == -999);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("Cb6/C6/B#1/", instrs, badb) == 0  &&  instrs == "OPZ"  &&  badb == -999);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("D/D4/C/C4/", instrs, badb) == 0  &&  instrs == "HHGG"  &&  badb == -999);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("C#4/Db4/", instrs, badb) == 0  &&  instrs == "!!"  &&  badb == -999);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("CDEFG/", instrs, badb) == 0  &&  instrs == "[GHJKL]"  &&  badb == -999);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("D2D3DD4D5/", instrs, badb) == 0  &&  instrs == "[X.HHR]"  &&  badb == -999);
    
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("D#Db/", instrs, badb) == 0  &&  instrs == "[@!]"  &&  badb == -999);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("D#4Db4/", instrs, badb) == 0  &&  instrs == "[@!]"  &&  badb == -999);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("C/D/E//CDE/", instrs, badb) == 0  &&  instrs == "GHJ [GHJ]"  &&  badb == -999);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("CDE/FGA/EF/", instrs, badb) == 0  &&  instrs == "[GHJ][KLQ][JK]"  &&  badb == -999);
    instrs = "xxx"; badb = -999; // so we can detect whether these get changed
    assert(translateTune("CD/F/CD//CD/", instrs, badb) == 0  &&  instrs == "[GH]K[GH] [GH]"  &&  badb == -999);
