...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_KILL: {
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
			...
			if (proc_array[p].p_blocked != -1) {
				proc_array[proc_array[p].p_blocked].p_state = P_RUNNABLE;
				proc_array[proc_array[p].p_blocked].p_registers.reg_eax = proc_array[p].p_exit_status;
				proc_array[p].p_blocked = -1;
			}
	...
	case INT_SYS_NEWTHREAD: {
		...
		proc_array[p].p_registers = current->p_registers;

		proc_array[p].p_registers.reg_esp = PROC1_STACK_ADDR + (p * PROC_STACK_SIZE);
		proc_array[p].p_registers.reg_eip = current->p_registers.reg_eax; // start function
