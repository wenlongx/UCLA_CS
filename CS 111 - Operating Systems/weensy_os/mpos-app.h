 ...
 * sys_kill(pid)
 *	
 *	Causes process with matching pid to exit. Returns the exit status of
 *	terminated process, or -1 if failed to terminate process.
 *
 */
 ...
 *	
 *	Creates a new thread with an empty stack. Returns the pid of the
 *	thread created, of -1 if failed to create a thread.
 */
